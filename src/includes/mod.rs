...
use sqlx::PgPool;
use teloxide::{
    Bot, dispatching::{HandlerExt, UpdateFilterExt}, 
    dptree, prelude::{Dispatcher, ResponseResult, Update, Message}, 
...
use crate::includes::select_all_problems::{
    State, 
    start_dialogue
};
...
pub async fn start_bot(pool: PgPool) -> ResponseResult<()> {
  ...
  let callback_handler_branch = dptree::entry() 
     .branch(Update::filter_callback_query().endpoint(callback_handler));

  let inline_branch_handler = dptree::entry() 
     .branch(Update::filter_inline_query().endpoint(inline_handler));

  let dialogue_handler = dptree::entry() 
      .branch(Update::filter_message()
        .enter_dialogue::<Message, InMemStorage<State>, State>()
        .branch(dptree::case![State::Start]).endpoint(start_dialogue)
    );

  let handler = dptree::entry()
    .branch(Update::filter_message().filter_command::<commands_handler::Commands>()
        .enter_dialogue::<Message, InMemStorage<State>, State>()
